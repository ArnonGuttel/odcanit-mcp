...
`scripts/mcp.exe.json` is a `.mcp.json` template for Claude Code users running the standalone `.exe` (as opposed to the root [`.mcp.json`](.mcp.json), which targets a source build via `node dist/index.js`). It's copied into the release zip as `.mcp.json`, alongside the `.exe` it references by relative path (`./odcanit-mcp.exe`), so both sit in the same flat folder. Its `env` block lists every connection param `setup-windows.ps1` collects (`ODCANIT_DB_HOST`, `ODCANIT_DB_PORT`, `ODCANIT_DB_INSTANCE`, `ODCANIT_DB_NAME`, `ODCANIT_DB_USER`, `ODCANIT_DB_PASSWORD`) as placeholders for the user to fill in by hand — unlike `setup-windows.ps1`, nothing writes into this file automatically, since it's the Claude Code path, not the Claude Desktop one. `ODCANIT_DB_PORT` and `ODCANIT_DB_INSTANCE` are mutually exclusive (see `db.ts` in Architecture below): the template defaults to the common non-named-instance case (`PORT` set, `INSTANCE` empty) — a user with a named instance must clear `PORT` when they set `INSTANCE`. Keep its env block in sync with whatever `setup-windows.ps1` collects, and keep it listed alongside the other release files below.
...
`.github/workflows/build-exe.yml` runs this pipeline in CI on every push/PR to `main` (as a build-only sanity check, uploaded as a workflow artifact) and, when a `v*` tag is pushed, additionally flattens `dist-bin/odcanit-mcp.exe`, `scripts/setup-windows.ps1`, `scripts/Setup.bat`, `scripts/Uninstall.bat`, and `scripts/mcp.exe.json` (renamed to `.mcp.json`) into a single `release/` folder (no subfolders — matching `setup-windows.ps1`'s same-directory lookup above) and publishes the zipped result as a GitHub Release via `softprops/action-gh-release` — this is what end users download per the README's Windows setup instructions.
