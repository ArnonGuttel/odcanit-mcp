...
- `DB_ENABLE_WRITES` (default `false`, no `ODCANIT_` prefix by design) — reserved for future write tools. `db.ts` exports `assertWritesEnabled()`, which throws unless this is `"true"`; write-capable tools should call it before executing. No tool calls it today.
